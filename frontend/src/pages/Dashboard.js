    ...
    <div className="dashboard-container" onClick={() => showAdminMenu && setShowAdminMenu(false)}>
      ...
      <aside className={`sidebar ${isCollapsed ? 'collapsed' : ''}`}>
        ...
        <nav className="flex-grow-1">
          {adminData.staffRole ? (
            <>
              {adminData.permissions?.canAccessDashboard && <NavItem active={activeTab === 'dashboard'} icon="grid-fill" label="Dashboard" onClick={() => setActiveTab('dashboard')} collapsed={isCollapsed} />}
              {adminData.permissions?.canAccessStudents && <NavItem active={activeTab === 'students'} icon="people" label="Students" onClick={() => setActiveTab('students')} collapsed={isCollapsed} />}
              {adminData.permissions?.canAccessAttendance && <NavItem active={activeTab === 'attendance'} icon="calendar-check" label="Attendance" onClick={() => setActiveTab('attendance')} collapsed={isCollapsed} />}
              {adminData.permissions?.canAccessBus && <NavItem active={activeTab === 'bus-boarding'} icon="bus-front" label="Bus Boarding" onClick={() => setActiveTab('bus-boarding')} collapsed={isCollapsed} />}
              {adminData.permissions?.canAccessLibrary && <NavItem active={activeTab === 'library'} icon="book" label="Library" onClick={() => setActiveTab('library')} collapsed={isCollapsed} />}
              {adminData.permissions?.canAccessPayments && <NavItem active={activeTab === 'payments'} icon="credit-card" label="Payments" onClick={() => setActiveTab('payments')} collapsed={isCollapsed} />}
